...
async function updateJSDocInChangedFiles() {
  ...
  for (const sourceFile of project.getSourceFiles()) {
    ...
    for (const jsdoc of jsdocs) {
      const buildTag = jsdoc
        .getTags()
        .find((tag) => tag.getTagName() === "build");
      const updatedTag = jsdoc
        .getTags()
        .find((tag) => tag.getTagName() === "updated");
      ...
      if (buildTag) {
        ...
        if (commentText) {
          ...
          if (!isNaN(currentBuild)) {
            ...
            console.log(
              `  -> File: ${sourceFile.getFilePath()}, @build updated from ${currentBuild} to ${newBuild}`,
            );
          }
        }
      }
      ...
      if (updatedTag) {
        ...
        fileWasModified = true;
        console.log(
          `  -> File: ${sourceFile.getFilePath()}, @updated changed to ${formattedDate}`,
        );
      }
    }
