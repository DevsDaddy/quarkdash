...
export class SHA512 {
    ...
    static hash(
        ...
        for (let i = 0; i < padded.length; i += 128) {
            ...
            for (let j = 16; j < 80; j++) {
                const s0 =
                    this.rightRotate(w[j - 15], 1) ^
                    this.rightRotate(w[j - 15], 8) ^
                    (w[j - 15] >> 7n);
                const s1 =
                    this.rightRotate(w[j - 2], 19) ^
                    this.rightRotate(w[j - 2], 61) ^
                    (w[j - 2] >> 6n);
            ...
            for (let j = 0; j < 80; j++) {
                const S1 =
                    this.rightRotate(e, 14) ^
                    this.rightRotate(e, 18) ^
                    this.rightRotate(e, 41);
                const ch = (e & f) ^ (~e & g);
                const temp1 = (h + S1 + ch + this.K[j] + w[j]) & 0xffffffffffffffffn;
                const S0 =
                    this.rightRotate(a, 28) ^
                    this.rightRotate(a, 34) ^
                    this.rightRotate(a, 39);
                const maj = (a & b) ^ (a & c) ^ (b & c);
