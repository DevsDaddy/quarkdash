...
describe("QuarkDash crypto algorithm tests", () => {
    ...
    test("Simple UTF-8 Text Encryption using Gimli Cipher", async () => {
        // Encrypt at client / decrypt at server
        ...
        const ciphertext = (await alice.initializeSession(
            bobPub,
            true,
        )) as Uint8Array;
        await bob.initializeSession(alicePub, false);
        ...
        const enc = await alice.encrypt(plain);
        const dec = await bob.decrypt(enc);
        const enc3 = await bob.encrypt(plain);
        const dec3 = await alice.decrypt(enc3);
        console.log("Decrypted Text:", QuarkDashUtils.bytesToText(dec));
        console.log("Decrypted Text:", QuarkDashUtils.bytesToText(dec3));

        const enc2 = await bob.encrypt(plain);
        const dec2 = await alice.decrypt(enc2);
        console.log(QuarkDashUtils.bytesToText(dec2));
